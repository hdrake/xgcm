...
Most finite volume ocean models use [Arakawa Grids], in which different
...
![C-grid Geometry](images/grid2d_hv.svg)

*Layout of variables with respect to cell centers and edges in a C-grid
ocean model. Image from the
[pycomodo project](https://web.archive.org/web/20160417032300/http://pycomodo.forge.imag.fr/norm.html).*

These grids present a dilemma for the [xarray] data model. The `u` and `t`
...
We use [MITgcm notation] to denote the basic operators that transform between
...
where $\Phi$ is any variable and `i` represents the grid index.
...
[Dirichlet](https://en.wikipedia.org/wiki/Dirichlet_boundary_condition), and
[Neumann](https://en.wikipedia.org/wiki/Neumann_boundary_condition) boundary
conditions, although the latter two are limited to simple cases, see [Boundary conditions](boundary_conditions.md).
...
## Axes and Positions {#axis-positions}
...
![axis positions](images/axis_positions.svg)
...
To begin, let's create a simple example `xarray.Dataset` with
...
Here we passed `boundary="periodic"` because this example data is periodic
(wrap-around) along the x axis. If the `boundary` keyword argument is not
...
until a `boundary` is supplied (e.g. `boundary="periodic"`, `"fill"`, or
...
favor of the more explicit `boundary` argument.
...
that follow the netCDF data model, and the [COMODO conventions] and
[SGRID conventions], both of which define
...
```python
grid = xgcm.Grid(ds, boundary="periodic")
grid
```
...
lat-lon-cap.
This is described in the [Grid Topology](grid_topology.md) page.
...
[SGRID Conventions]: https://sgrid.github.io/sgrid/
